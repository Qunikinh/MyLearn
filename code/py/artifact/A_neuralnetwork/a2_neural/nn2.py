...
def main(lr=0.01, epochs=100):
    ...
    # 整体进度条
    overall_pbar = tqdm(total=epochs, desc="训练中", ncols=100, colour='green')
    ...
    print(f"学习率：{lr} 训练轮数：{epochs} 计算方式：{device}")
    print(f"训练集损失：{train_losses[-1]:.3f} 验证集损失：{val_losses[-1]:.3f} 测试集损失：{test_loss:.3f}")
